...
<body>
	...
	<c:if test="${not empty requestScope.admins}">
		<div class="container-title">Administrators</div>
	</c:if>
	...
	<c:if test="${not empty requestScope.delUsers}">
		<div class="container-title">Deactivated account</div>
	</c:if>
	
	<c:forEach items="${requestScope.delUsers}" var="user">
		...
		<div class="establishment" onclick="location.href='user?uid=${user.uid}'">
		     ...
		     <div class="esta-button">
		     <a class="esta-delete" href="userdelete?uid=${user.uid}&hard=true">Delete</a>
		     <a class="esta-edit" href="userdelete?uid=${user.uid}&undo=true">Enable</a>
		   </div>
